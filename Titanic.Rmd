...
lm.pred <- predict(lm, newdata = test4.df)
...
for(t in ntrees){
  ...
  train.points.df <- train4.df[train.points, ]
  test.points.df <- train4.df[-train.points, ]
  train.points.input <- as.data.frame(train.points.df[, -2])
  train.points.output <- as.vector(as.matrix(train.points.df[, 2]))
  test.points.input <- test.points.df[, -2]
  test.points.output <- test.points.df[, 2]
  
  rf.mod <- randomForest(train.points.input, train.points.output, ntree = t)
  rf.cv <- rf.crossValidation(rf.mod, train.points.input, ntree = t)
}
